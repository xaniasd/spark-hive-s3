...
# INSPIRED FROM: https://github.com/mjwall/apache-bash-downloader/blob/master/common.sh
...
wait_for() {
  ...
  blue "Waiting for ${host}:${port} for max ${timeout} seconds"
  ...
  abort "Timed out waiting for ${host}:${port}"
}
